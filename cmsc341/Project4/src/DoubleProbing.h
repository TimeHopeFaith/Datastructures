...
*
* This is the Double Probing class that will use the double hashing algorithm 
* for insertion, deletion, etc.
...
public:
	...
	explicit DoubleProbing(unsigned int size, int r) : array(size) , currentSize(size), prime(r)
	{ 	
		numClusters = 0;
	...
	int secondHash(const HashedObj & x ) const
	{
		int hashVal = doubleHash( x );
	...
	int findPos( const HashedObj & x )
	{
		...
		{	
			...
			currentPos = (originalPos + (offset * hashTwo)) % (int)array.capacity();
			offset++;
	...
	void PrintTable()
	{
		cout << "Double Probing actual table" << endl;
...
};// end DoubleProbing class definition
