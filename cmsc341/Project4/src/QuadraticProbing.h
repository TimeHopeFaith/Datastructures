...
* Description:
*
* IMPORTANT: MUST CALL CalculateClusters() before accessing for any cluster values
*		to ensure current cluster sizes are accurate
*
* This is the Quadratic hash that uses the algorithm 
* h( K, I ) = ( h( K ) + I^2) mod M 
* for probing and filling the table
*
...
public:
	...
	explicit QuadraticProbing(unsigned int size ) : array(size) , currentSize(size)
	{ 
		numClusters = 0;
	...
	void PrintTable()
	{
		cout << "Quadratic Probing actual table" << endl;
...
};// end QuadraticProbing class definition
